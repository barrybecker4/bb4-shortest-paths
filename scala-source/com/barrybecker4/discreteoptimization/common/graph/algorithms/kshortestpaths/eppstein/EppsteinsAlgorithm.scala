 ...
 *
 * Modified from https://github.com/bsmock/k-shortest-paths/blob/master/edu/ufl/cise/bsmock/graph/ksp/Eppstein.java
 */
class EppsteinsAlgorithm(graph: DirectedGraph) extends KShortestPathsFinder {
   ...
   */
  def ksp(source: Int, target: Int, K: Int): Seq[Path] = {
    /* Compute the shortest path tree, T, for the target node (the shortest path from every node in the graph to the target) */
    var tree: ShortestPathTree = _
    try {
      tree = new DijkstrasAlgorithm(graph).findShortestPaths(graph.transpose, target) // convert ShortestPaths to ShortestPathTree
    }
    catch {
      case e: Exception =>
        tree = new ShortestPathTree(target)
    }
    // Compute the set of sidetrack edge costs
    var sidetrackEdgeCostMap: Map[String, Double] = computeSidetrackEdgeCosts(tree)
    /* Make indexes to give fast access to these heaps later */
    // Heap H_out(v) for every node v
    var nodeHeaps: Map[String, EppsteinHeap] = Map() // initial size graph.numNodes
    var edgeHeaps: Map[String, EppsteinHeap] = Map() // initial size graph.numEdges
    // Heap H_T(v) for every node v
    val outrootHeaps: Map[String, EppsteinHeap] = Map()
    ...
    for (nodeIdx <- 0 until graph.numNodes) {
      computeOutHeap(nodeIdx, sidetrackEdgeCostMap, nodeHeaps, edgeHeaps)
    }
    ...
    val edges = tree.nodes.map(node => DirectedEdge(node.parent, node.idx, graph.getNeighborsOf(node.idx)))
//    for (node <- tree.getNodes.values) {
//      reversedSPT.addEdge(node.getParent, node.getLabel, graph.getNode(node.getLabel).getNeighbors.get(node.getParent))
//    }
    val reversedSPT = DirectedGraph(graph.numNodes, edges)
    ...
    val rootArrayHeap = EppsteinArrayHeap()

    // Perform the DFS (recursively initiating additional depth-first searches for each child)
    recursiveOutrootHeaps(target, rootArrayHeap, nodeHeaps, outrootHeaps, reversedSPT)
    ...
    val hg: Nothing = new Nothing(new Nothing(sourceLabel, sourceLabel, 0))
    // Initialize the containers for the candidate k shortest paths and the actual found k shortest paths
    val ksp: Nothing = new Nothing
    val pathPQ: Nothing = new Nothing
    // Place root heap in priority queue
    pathPQ.add(new Nothing(hg, -(1), tree.getNodes.get(sourceLabel).getDist))
    /* Pop k times from the priority queue to determine the k shortest paths */
    var i: Int = 0
    while (i < K && pathPQ.size > 0) {
      ...
      val kpathImplicit: Nothing = pathPQ.poll
      // Convert from the implicit path representation to the explicit path representation
      val kpath: Nothing = kpathImplicit.explicitPath(ksp, tree)
   ...
   * @return
   */
  protected def computeSidetrackEdgeCosts(graph: Nothing, tree: Nothing): Nothing = {
    val sidetrackEdgeCostMap: Nothing = new Nothing
    val edgeList: Nothing = graph.getEdgeList
    ...
    for (edge <- edgeList) {
      ...
      val tp: String = tree.getParentOf(edge.getFromNode)
      if (tp == null || !(tp == (edge.getToNode))) {
        val sidetrackEdgeCost: Double = edge.getWeight + tree.getNodes.get(edge.getToNode).getDist - tree.getNodes.get(edge.getFromNode).getDist
   ...
   */
  protected def computeOutHeap(nodeLabel: String, graph: Nothing, sidetrackEdgeCostMap: Nothing, nodeHeaps: Nothing, edgeHeaps: Nothing): Unit = {
    val node: Nothing = graph.getNode(nodeLabel)
    // This list holds the 2nd through last sidetrack edges, ordered by sidetrack cost
    val sidetrackEdges: Nothing = new Nothing
    var bestSidetrack: Nothing = null
    var minSidetrackCost: Double = Double.MAX_VALUE
    // Iterate over the outgoing edges of v
    import scala.collection.JavaConversions._
    for (neighbor <- node.getAdjacencyList) {
      val edgeLabel: String = nodeLabel + "," + neighbor
      // Check to see if the current edge is a sidetrack edge
      if (sidetrackEdgeCostMap.containsKey(edgeLabel)) {
        val sidetrackEdgeCost: Double = sidetrackEdgeCostMap.get(edgeLabel)
        ...
        if (sidetrackEdgeCost < minSidetrackCost) {
          ...
          bestSidetrack = new Nothing(nodeLabel, neighbor, node.getNeighbors.get(neighbor))
          minSidetrackCost = sidetrackEdgeCost
        }
        else {
          sidetrackEdges.add(new Nothing(nodeLabel, neighbor, node.getNeighbors.get(neighbor)))
        }
      }
    }
    ...
    if (bestSidetrack != null) {
      ...
      // root
      val bestSidetrackHeap: Nothing = new Nothing(bestSidetrack, sidetrackEdgeCostMap.get(bestSidetrack.getFromNode + "," + bestSidetrack.getToNode))
      // Make another heap (a binary heap) out of the rest of the sidetrack edges of v
      val arrayHeap: Nothing = new Nothing
      if (sidetrackEdges.size > 0) {
        bestSidetrackHeap.setNumOtherSidetracks(bestSidetrackHeap.getNumOtherSidetracks + 1)
        import scala.collection.JavaConversions._
        for (edge <- sidetrackEdges) {
          val sidetrackHeap: Nothing = new Nothing(edge, sidetrackEdgeCostMap.get(edge.getFromNode + "," + edge.getToNode))
   ...
   */
  protected def addExplicitChildrenToQueue(kpathImplicit: Nothing, ksp: Nothing, pathPQ: Nothing): Unit = {
    val kpathCost: Double = kpathImplicit.getCost
    import scala.collection.JavaConversions._
    for (childHeap <- kpathImplicit.getHeap.getChildren) {
      // Get the index of the previous shorter path off of which this candidate sidetracks/branches
      val prefPath: Int = kpathImplicit.getPrefPath
      // Calculate the path cost of the new child/candidate
      val candidateCost: Double = ksp.get(prefPath).getTotalCost + childHeap.getSidetrackCost
      // Add the child/candidate to the priority queue
      val candidate: Nothing = new Nothing(childHeap, prefPath, candidateCost)
   ...
   * @param pathPQ        priority queue of candidate paths
   */
  protected def addCrossEdgeChildToQueue(outrootHeaps: Nothing, kpathImplicit: Nothing, prefPath: Int, ksp: Nothing, pathPQ: Nothing): Unit = {
    if (outrootHeaps.containsKey(kpathImplicit.getHeap.getSidetrack.getToNode)) {
      val childHeap: Nothing = outrootHeaps.get(kpathImplicit.getHeap.getSidetrack.getToNode)
      // Calculate the path cost of the new child/candidate
      val candidateCost: Double = ksp.get(prefPath).getTotalCost + childHeap.getSidetrackCost
      // Add the child/candidate to the priority queue
      val candidate: Nothing = new Nothing(childHeap, prefPath, candidateCost)
   ...
   */
  protected def recursiveOutrootHeaps(nodeLabel: String, currentArrayHeap: Nothing, nodeHeaps: Nothing, outrootHeaps: Nothing, reversedSPT: Nothing): Unit = {
    // Get H_out(v)
    val sidetrackHeap: Nothing = nodeHeaps.get(nodeLabel)
    ...
    val currentHeap: Nothing = currentArrayHeap.toEppsteinHeap2
    ...
    import scala.collection.JavaConversions._
    for (neighbor <- reversedSPT.getNode(nodeLabel).getNeighbors.keySet) {
