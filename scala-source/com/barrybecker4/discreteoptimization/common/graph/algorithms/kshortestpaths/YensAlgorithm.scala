...
class YensAlgorithm(graph: DirectedGraph) extends KShortestPathsFinder {
  ...
  private def searchAll(start: Int, end: Int, k: Int): List[Path] = {
    if (paths.isEmpty && pathCandidates.isEmpty) {
      // initialize by getting the shortest path
      ...
      val nextDerivedNodeSeq = nextPath.nodes.indexOf(nextDerivedNode)
      val nextSubPathNodeList: List[Int] = nextPath.nodes.dropRight(nextPath.nodes.size - nextDerivedNodeSeq - 1)
      ...
      // 2. recover from the very ending, and calculate a few more candidates
